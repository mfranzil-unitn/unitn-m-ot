...
echo 1638400 1638400 1638400 > /proc/sys/net/ipv4/tcp_mem
...
iptables -t mangle -A PREROUTING -p tcp -m conntrack --ctstate NEW -m tcpmss ! --mss 1300:1500 -j DROP

#iptables -t raw -A PREROUTING -p tcp -m conntrack --ctstate NEW ! --tcp-option 4 -j DROP
...
iptables -t raw -A PREROUTING -p tcp --tcp-flags SYN,ACK,FIN,RST RST -j DROP  
...
#IP WHITELIST
#iptables -t raw -A PREROUTING -i $router_iface ! -s 10.1.0.0/16 -p tcp -j DROP
iptables -t raw -A PREROUTING -i $router_iface -s 10.1.1.2,10.1.3.2,10.1.4.2,10.1.2.2 -p tcp -j ACCEPT
iptables -t raw -A PREROUTING -i $router_iface -p tcp -j DROP
...
# Limit RST packets # 
iptables -A FORWARD -p tcp --tcp-flags RST RST -m limit --limit 2/s --limit-burst 2 -j ACCEPT
