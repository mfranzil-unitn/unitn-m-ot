...
void ExampleProcess(void *pkt, void *context) {
    ...
    if (p->dst_port == 7777) {
        char buf[p->payload_size];
        int actual_size = 0;

        // Copy payload to buf with translation
        for (int i = 0; i < p->payload_size; i++) {
            if (p->payload[i] == '%') { // If we're near the "feature"
                // Check the three following characters and verify that it's a number between 0 and 255
                if (i + 3 < p->payload_size && isdigit(p->payload[i + 1]) && isdigit(p->payload[i + 2]) && isdigit(p->payload[i + 3])) {
                    // Convert the three characters to a number
                    int num = (p->payload[i + 1] - '0') * 100 + (p->payload[i + 2] - '0') * 10 + (p->payload[i + 3] - '0');
                    // If the number is between 0 and 255, add it to the buffer
                    if (num >= 0 && num <= 255) {
                        buf[actual_size++] = (char)num;
                        i += 3; // Skip the three characters
                    } else {
                        // If the number is not between 0 and 255, just add the %
                        buf[actual_size++] = '%';
                    }
                } else {
                    // If the number is not between 0 and 255, just add the %
            ...
            } else { // Just straight out copy
                // normal copy
                buf[actual_size++] = p->payload[i];
            }
        }
